...
export default class PacManTest extends Test {
	...
	public setStoppedAtTurnTest(): void {
		const pacman = new PacMan();

		this.assertOfType("undefined", pacman["stoppedAtTurn"]);

		const turn = new Turn("test-turn", []);

		pacman.setStoppedAtTurn(turn);

		this.assertStrictlyEqual(turn, pacman["stoppedAtTurn"]);
	}
	...
	public startMovingTest(): void {
		...
		pacman.setStoppedAtTurn(new Turn("test-turn", []));
	...
	/**
	 * Test that pacman instances can handle input from the player.
	 */
	public handleInputTest(): void {
		const pacman = new PacMan();
		const direction = MovementDirection.RIGHT;

		this.assertFalse(pacman["moving"]);
		this.assertTrue(pacman["spawning"]);
		this.assertOfType("undefined", pacman["currentDirection"]);

		pacman.stopMoving();

		pacman["_currentInputDirection"] = direction;

		pacman["handleInput"]("KeyD");

		// should be able to move off spawn
		this.assertFalse(pacman["spawning"]);

		pacman.setStoppedAtTurn(new Turn("test-turn", [direction]));
		pacman["handleInput"]("KeyD");

		// should be able to move when stopped at turn
		this.assertTrue(pacman["moving"]);
		this.assertStrictlyEqual(direction, pacman["currentDirection"]);
	}
